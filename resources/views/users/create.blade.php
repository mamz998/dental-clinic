...
<div class="card" style="max-width:620px;margin:0 auto">
    <div class="card-header">
        <span class="card-title">👤 إضافة مستخدم جديد</span>
        ...
        <form method="POST" action="{{ route('users.store') }}">
            ...
            <div style="margin-bottom:16px">
                ...
                <select class="form-input" name="role" id="role-select" onchange="toggleDoctorFields()" required>
                    ...
                    <option value="receptionist" {{ old('role')==='receptionist'?'selected':'' }}>استقبال</option>
                </select>
            </div>
            ...
            <div id="doctor-fields" style="display:none;background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:16px;margin-bottom:16px">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                    <div>
                        ...
                        <input class="form-input" name="specialty" value="{{ old('specialty') }}" placeholder="مثال: تقويم، تركيبات...">
                    </div>
                    <div>
                        <label class="form-label">نسبة العمولة %</label>
                        <input class="form-input" type="number" name="commission_rate" value="{{ old('commission_rate',0) }}" min="0" max="100" step="0.5">
                        <p style="font-size:11px;color:#6b7280;margin-top:4px">النسبة من إجمالي فواتير المريض</p>
                    </div>
                </div>
            </div>

            <div style="display:flex;align-items:center;gap:10px;margin-bottom:24px">
                <input type="checkbox" name="is_active" value="1" id="is_active" checked style="width:16px;height:16px;accent-color:#3b82f6">
                <label for="is_active" class="form-label" style="margin:0">حساب نشط</label>
            </div>

            <button type="submit" class="btn btn-primary" style="width:100%;padding:13px">إضافة المستخدم</button>
        </form>
    </div>
</div>
